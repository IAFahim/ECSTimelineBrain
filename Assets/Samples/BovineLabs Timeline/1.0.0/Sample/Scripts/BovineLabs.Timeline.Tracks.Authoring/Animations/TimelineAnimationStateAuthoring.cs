...
namespace BovineLabs.Timeline.Authoring
{
    public class TimelineAnimationStateAuthoring : MonoBehaviour
    {
        ...
        public float blendInDuration = 0.25f;[Tooltip("Time in seconds to smoothly transition from Timeline back to Fallback.")][Min(0.001f)]
        ...
        public class Baker : Baker<TimelineAnimationStateAuthoring>
        {
            public override void Bake(TimelineAnimationStateAuthoring authoring)
            {
                ...
                if (authoring.fallbackAnimationClip != null)
                {
                    ...
                    var bakedAnimations = animationBaker.BakeAnimations(this, new[] { authoring.fallbackAnimationClip }, avatar, authoring.gameObject);
                    ...
                    {
                        dbBuffer.Add(new NewBlobAssetDatabaseRecord<AnimationClipBlob> { hash = fallbackHash, value = bakedAnimations[0] });
                    }
                    if (bakedAnimations.IsCreated) bakedAnimations.Dispose();
                }

                AddComponent(entity, new BlendGroupTimer { TimelineWeight = 0f, FallbackAccumulatedTime = 0f });
                ...
                AddBuffer<PreviousBlendGroupEntry>(entity); // Added to support smooth fadeouts
            }
        }
    }
}