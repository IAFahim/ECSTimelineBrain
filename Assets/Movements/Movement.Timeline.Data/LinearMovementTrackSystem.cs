...
namespace Movements.Movement.Timeline.Data
{
    ...
    public partial struct LinearMovementTrackSystem : ISystem
    {
        ...
        {
            positionImpl.OnCreate(ref state);
            rotationImpl.OnCreate(ref state);
        }

        [BurstCompile]
        public void OnDestroy(ref SystemState state)
        {
            positionImpl.OnDestroy(ref state);
            rotationImpl.OnDestroy(ref state);
        }
        ...
        public void OnUpdate(ref SystemState state)
        {
            ...
            var positionBlend = positionImpl.Update(ref state);
            ...
            var rotationBlend = rotationImpl.Update(ref state);
        ...
        private partial struct ResolveTargetPositionJob : IJobEntity
        {
                ...
                ref LinearMovementAnimated animated)
            {
                ...
                if (LocalTransforms.TryGetComponent(target.Target, out var localTransform))
        ...
        private partial struct CaptureStartJob : IJobEntity
        {
            ...
            {
                if (!LocalTransforms.TryGetComponent(trackBinding.Value, out var bindingTransform))
                {
        ...
        private partial struct ResetJob : IJobEntity
        {
                ...
                in TrackBinding trackBinding)
            {
                ...
                var lt = LocalTransforms.GetRefRWOptional(trackBinding.Value);
                if (!lt.IsValid) return;

                if (reset.ResetPosition) lt.ValueRW.Position = startPos.Position;

                if (reset.ResetRotation) lt.ValueRW.Rotation = startPos.Rotation;
            }
        }
